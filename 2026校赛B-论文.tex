...
针对问题二，将场景扩展至班组 1 独立完成 A、B、C、D、E 五个车间的全部整修任务，构建了带序列相关转移时间与协同加工约束的资源受限车间调度整数规划模型。以最小化总完工时间为目标，引入路由变量刻画各类设备在工序间的访问顺序，采用大 $M$ 法建立资源析取与跨车间转运约束。提出遗传算法与模拟退火级联生成可行上界\cite{shi2026}、以 \texttt{AddHint} 热启动注入 CP-SAT 求解器的两层混合求解框架，由约束规划完成零间隙最优性证明，得到最短完工时间为 $144880\,\mathrm{s}$。
...
\textbf{推荐参数}：$N_{\mathrm{pop}}=150$，$G=800$，$p_m=0.2$，$T_0=4000$，$\alpha=0.97$，$N_{\mathrm{iter}}=400$。该组合在 GA+SA 耗时 $11.9\,\mathrm{s}$ 内稳定生成零间隙上界，热启动 CP-SAT 仅需 $0.2\,\mathrm{s}$ 即完成最优性证明。增大种群与代数至 $200/800$ 虽可进一步缩短热启动耗时至 $0.1\,\mathrm{s}$，但元启发式耗时增至 $15.3\,\mathrm{s}$，总耗时反超推荐参数，故不采纳。
...
由表~\ref{tab:coldhot} 可见：三个问题的冷/热启动所得最优值完全一致，验证了 Hint 仅引导搜索方向、不裁剪可行解空间的性质；热启动求解耗时均短于冷启动，加速比为 $1.4\times$--$2.1\times$。由于本文所涉问题规模紧凑（$|\mathcal{O}|=34$），CP-SAT 冷启动已可在亚秒级收敛，故加速比的绝对值有限。但对于更大规模或更严苛时限下的调度实例，Hint 热启动降低搜索盲目性的收益将更为显著。
...
\midrule
二 & $144880$ & $144880$ & $0$ & 通过 & $1.89$ \\
三 & $73575$ & $73575$ & $0$ & 通过 & $1.41$ \\
四 & $32608$ & $32608$ & $0$ & 通过 & $2.12$ \\
\bottomrule
\end{tabular}
\end{table}

GA 与 SA 具有随机性：固定推荐参数下，部分随机种子仍可能收敛至次优值（如个别种子给出 $148051\,\mathrm{s}$ 的上界，间隙 $3171\,\mathrm{s}$），但多次运行可稳定复现精确最优值。本文以 CP-SAT 零间隙证明为最优性的最终依据——元启发式层提供上界与搜索加速，CP-SAT 承担精确证明职责。所有调度结果均经独立可行性校验器 \texttt{validate.py} 逐项核验（先后约束、设备不重叠、转运充足、双设备完工），确保无"表观最短但不可行"的解。各问设备利用率热力图见图~\ref{fig:util}。
