...
问题一在单车间串行结构下解析求得 $C_{\max}=37280\,\mathrm{s}$。问题二至四采用\textbf{两层混合求解}：遗传算法与模拟退火快速生成高质量可行上界，再以 \texttt{AddHint} 将开工时刻、完工时刻、路由弧注入 CP-SAT 热启动，由约束规划完成零间隙最优性证明。问题二得 $C_{\max}=144880\,\mathrm{s}$，问题三得 $73575\,\mathrm{s}$，问题四在预算 $500000$ 元下增购抛光机、传感机各 3 台（费用 $465000$ 元），得 $C_{\max}=32608\,\mathrm{s}$。网格调参与冷/热启动对比表明，推荐参数下元启发式可复现精确最优值，热启动使 CP-SAT 证明耗时缩短约 $30\%$--$50\%$。

\noindent\textbf{关键词：}\,作业车间调度；序列相关转移时间；混合求解；约束规划；遗传算法；模拟退火

%==================================================================
\section{问题背景与整体分析}

\subsection{问题背景}
现代制造与工业维护系统普遍呈现多工序协同、多资源共享的特征。某工业制造系统需对 A、B、C、D、E 五个车间集中开展整修。每个车间的整修任务可抽象为一条由若干工序组成的工艺链，工序之间存在严格的先后依赖；每道工序须占用一种或两种特定类型的设备完成，当某工序由两类设备协同完成时，两类设备须各自独立完成该工序对应的全部工程量，且以两者中较晚完成者作为该工序的完工时刻。设备在班组基地与各车间、以及车间相互之间的转移须计入与距离成正比的转运时间，而同一车间内不同工序之间的设备转移时间可忽略。系统配置自动化输送臂、工业清洗机、精密灌装机、自动传感多功能机、高速抛光机五类设备，分属两个班组，各类设备数量、移动速度与单价均已给定。本文需在上述时序、能力与转运约束下确定作业顺序与资源分配，使全部整修任务的总完工时间最短，并进一步在给定预算下确定设备购置方案。

\subsection{问题类型判定}
四个子问题均要求在一组离散约束下最小化总完工时间，决策对象为工序的开工时刻、设备到工序的分配以及设备在车间间的转移顺序。其数学本质是带序列相关转移时间与协同加工约束的资源受限调度优化，属于\textbf{优化类}问题中的组合优化与整数规划范畴；问题四在调度之上叠加了预算约束下的资源购置决策，构成"购置—调度"耦合的双层优化，但其内核仍为优化类。本文不将其纳入预测、评价、数理统计或机理分析等范畴，因为问题既不涉及由样本到总体的统计推断，也不涉及由物理机理推导的微分方程描述，而是以可行域上的极值搜索为唯一目标。

\subsection{整体求解思路与递进逻辑}
四个子问题在规模与决策维度上层层递进，本文据此设计一条贯通的建模主线，使各问的模型在结构上保持一致、在能力上逐步增强，如图所示的逻辑链：单链可分负载分配 $\rightarrow$ 单班组聚合作业车间调度 $\rightarrow$ 双班组资源池化调度 $\rightarrow$ 预算约束下的购置—调度双层优化。

第一阶段（问题一）剥离跨车间转运与多车间竞争，仅保留"工序串行、同类设备并行分担工程量"这一核心机理。由于 A 车间三道工序严格串行，瓶颈完全由单台设备工序（高速抛光机完成的表面整平、自动传感多功能机完成的强度检测）决定，故可不借助一般调度框架而以解析方式给出最优解，并为后续问题确立"工时计算口径"与"可分负载假设"。

第二阶段（问题二）将场景扩展至五个车间共享一套班组 1 设备，问题成为带序列相关转移时间与协同加工的作业车间调度。本文对资源建立聚合抽象，设计 GA/SA 上界生成与 CP-SAT Hint 热启动的两层混合求解框架。

第三阶段（问题三）在问题二的模型结构上引入班组 2。两班组同类设备合并为统一资源池，瓶颈资源能力随之翻倍，初始转运取两基地到首作业车间的较大值以反映协同搬运。模型形式与问题二完全一致，仅资源容量与初始转运参数发生变化，体现了建模框架的可扩展性。

第四阶段（问题四）在问题三之上叠加预算约束下的资源购置决策。本文先以资源载荷下界识别瓶颈，论证仅瓶颈设备的增购才可能改善总工期，从而将高维购置组合压缩至两类瓶颈设备；继而建立外层枚举可行购置组合、内层对每个组合精确求解调度的双层优化模型，在 $500000$ 元预算内确定使完工时间最短的购置方案。四个阶段层层嵌套、逐步稳健，构成一个完整且自洽的求解体系。
...
& s_{o'} \ge f_o, \quad \forall w,\ (o,o')\ \text{为}\ \mathcal{O}_w\ \text{相邻工序}, \label{eq:prec}\\
...
& \text{（路由 $x^{r}_{o,o'}$ 构成一条从基地出发、遍历 $\mathcal{O}(r)$ 并返回基地的单回路，}\notag\\
&\quad\ \text{由 OR-Tools 的 \texttt{AddCircuit} 约束统一实现）}, \quad \forall r, \label{eq:circuit}\\
...
& s_o\ge 0,\quad f_o\ge 0,\quad x^{r}_{o,o'}\in\{0,1\}. \notag
\end{align}
其中 $\tau^{\mathrm{init}}_{w}=\max_{b\in\mathcal{B}}\tau_{b,w}$（取各基地到车间 $w$ 转运时间的最大值），问题二只有班组 1，故 $\mathcal{B}=\{\mathrm{G1}\}$，$\tau^{\mathrm{init}}_{w}=\tau_{\mathrm{G1},w}$；$M$ 为充分大的常数，本文取时间上界 $H$（所有工序工期之和加冗余），即当路由弧未激活（$x^{r}_{o,o'}=0$）时使对应不等式自动松弛。
...
\item \textbf{总完工时间定义 \eqref{eq:cmax}}：$C_{\max}$ 不小于任意工序的完工时刻 $f_o$，配合目标函数 $\min C_{\max}$，等价于最小化最晚完工时间。

\item \textbf{变量取值范围}：开工时刻与完工时刻为非负实数（或整数，由 CP-SAT 自动离散化到秒级），路由变量 $x^{r}_{o,o'}$ 为 0-1 布尔变量。
\end{enumerate}
...
\subsubsection{元启发式层与 Hint 热启动}
染色体为工序序编码；解码口径与模型一致（协同取 $\max_r p_{o,r}$、并行转场）。由 $\{s_o\}$ 反推各资源访问序构造路由弧 Hint。
