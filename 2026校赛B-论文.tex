...
\title{\zihao{3}\bfseries 多工序协同作业下的车间整修任务调度与资源分配优化}
\author{}
\date{}
...
本文围绕多工序协同作业环境下的车间整修任务调度与资源分配优化问题，基于\textbf{组合优化与约束规划理论}，建立了统一的\textbf{工序链—资源—转运网络模型}，系统研究了从单车间到多车间、从单班组到双班组及从固定资源配置到预算约束下资源购置决策等不同场景下的最优调度方案。本文全部数学模型的计算与模拟过程均通过 \textbf{Python 编程}实现（基于 \textbf{OR-Tools CP-SAT 求解器}及自编\textbf{遗传算法与模拟退火程序}），确保了算法的可执行性与结果的可复现性。

针对问题一，本文在单车间串行结构下，基于工序严格串行与同类设备并行分担工程量的核心机理，通过\textbf{解析计算}确定班组 1 完成 A 车间全部工序的最短时长。由于 A 车间仅含三道工序且无跨车间竞争，瓶颈完全由单台设备工序决定，通过投入全部可用台数并行分担协同工序工程量，解析求得最短完工时间为 \textbf{$37280\,\mathrm{s}$}，并给出了每台设备的作业时间线。

针对问题二，将场景扩展至班组 1 独立完成 A、B、C、D、E 五个车间的全部整修任务，构建了带\textbf{序列相关转移时间与协同加工约束}的资源受限车间调度整数规划模型。以最小化总完工时间为目标，引入\textbf{路由变量}刻画各类设备在工序间的访问顺序，采用\textbf{大 $M$ 法}建立资源析取与跨车间转运约束。提出\textbf{遗传算法与模拟退火级联}生成可行上界\cite{shi2026}、以 \textbf{\texttt{AddHint} 热启动}注入 CP-SAT 求解器的\textbf{两层混合求解框架}，由约束规划完成\textbf{零间隙最优性证明}，得到最短完工时间为 \textbf{$144880\,\mathrm{s}$}。

针对问题三，在问题二模型基础上引入班组 2 设备，将两班组同类设备合并为\textbf{统一资源池}，模型结构不变而资源容量翻倍，初始转运取两基地到各车间的较大值。求解得到最短完工时间为 \textbf{$73575\,\mathrm{s}$}，较问题二缩短约 \textbf{$49.2\%$}，接近资源翻倍的理想加速比。

针对问题四，在双班组基础上追加总额 $500000$ 元的设备购置预算，构建了\textbf{购置—调度耦合的双层优化模型}。外层以\textbf{资源载荷下界分析}将购置维度由五类设备严格降至\textbf{高速抛光机与自动传感多功能机}两类瓶颈设备，枚举可行购置组合；内层对每个组合调用混合求解框架精确求解。得到最优购置方案为增购高速抛光机与自动传感多功能机各 \textbf{3 台}（费用 \textbf{$465000$ 元}），最短完工时间为 \textbf{$32608\,\mathrm{s}$}，较问题三缩短约 \textbf{$55.7\%$}。冷启动与热启动对比实验表明，\texttt{AddHint} 热启动使 CP-SAT 求解耗时加速约 \textbf{$1.4\times$--$1.8\times$}。
...
四个子问题在规模与决策维度上层层递进：从单车间到五车间、从单班组到双班组、从固定配置到预算约束下的购置决策。本文据此设计\textbf{统一的建模框架}，使各问的模型在结构上保持一致、在能力上逐步增强。

\subsection{问题一的分析}

问题一要求仅使用班组 1 的设备完成 A 车间的全部整修任务，计算最短完工时长并给出每台设备的作业调度明细。A 车间含 \textbf{A1}（精密灌装机与自动化输送臂协同，$300\,\mathrm{m}^3$）、\textbf{A2}（高速抛光机与工业清洗机协同，$500\,\mathrm{m}^3$）、\textbf{A3}（自动传感多功能机，$500\,\mathrm{m}^3$）三道严格串行工序。由于仅涉单一车间，无跨车间设备竞争与转运，决策自由度仅在于同类多台设备是否全部投入协同工序的工程量分担。问题规模小、结构简单，可通过\textbf{解析方式}直接求得最优解，无需借助一般调度框架。该问题同时为后续多车间调度确立\textbf{工时计算口径}与\textbf{可分负载假设}。

\subsection{问题二的分析}

问题二要求仅使用班组 1 的设备，同时完成 A、B、C、D、E 五个车间的整修任务，其中 \textbf{C 车间的 C3--C5 工序需重复三遍}。此时间题升级为带\textbf{序列相关转移时间}（Sequence-Dependent Setup Time, SDST）与\textbf{协同加工约束}的多机作业车间调度：每个车间为一条固定工序链，每道工序占用一种或两种设备类型，同类设备全局串行服务，跨车间切换须预留转运时间。与经典柔性作业车间调度问题（FJSP）的差异在于：协同工序的工期由两类设备中较慢者决定，转移时间取决于设备前一作业车间与下一作业车间的距离，同类多台设备可并行分担工程量从而缩短单工序占用时间。本文对同类设备建立\textbf{聚合抽象（超级机器）}，将问题建模为每类设备在工序间的路由选择与时间排程问题，设计\textbf{元启发式生成上界、约束规划精确证明}的两层混合求解策略。

\subsection{问题三的分析}

问题三在问题二的基础上引入班组 2 的设备资源。两班组所配备的设备类型完全相同（五类设备各若干台），故自然地将同类设备合并为\textbf{统一资源池}：自动化输送臂增至 8 台、工业清洗机与精密灌装机各增至 10 台、自动传感多功能机与高速抛光机各增至 2 台。模型结构与问题二完全一致，仅资源容量参数与初始转运参数（取两基地到各车间转运时间的最大值）发生变化，体现了建模框架的\textbf{可扩展性}。由于瓶颈资源（高速抛光机、自动传感多功能机）翻倍，预期总工期将有显著下降。

\subsection{问题四的分析}

问题四在问题三的双班组资源池基础上，追加总额为 $500000$ 元的设备购置预算，需联合确定购置方案与调度策略使总完工时间最短。该问题构成\textbf{购置—调度耦合的双层优化}：外层决定增购哪些设备、各增购多少台，内层在给定设备配置下求解最优调度。五类设备均可增购，若直接枚举组合则搜索空间过大。本文利用\textbf{资源载荷下界分析}识别瓶颈设备，论证只有增购当前瓶颈设备才可能降低总工期下界，从而将外层搜索空间由\textbf{五维压缩至二维}（仅高速抛光机与自动传感多功能机），使枚举所有可行购置组合成为可能。
...
\item \textbf{同类设备聚合为超级机器}。将类型 $r$ 的 $n_r$ 台物理设备抽象为一台容量为 $n_r$ 的超级机器（super machine）。该超级机器以编组方式统一调度：同类设备共享同一工序访问序列，在同一工序上以全部 $n_r$ 台并行作业（合并效率 $n_r\cdot e_{o,r}$），同一时刻超级机器至多服务一道工序。该聚合抽象的合理性与边界分析如下：
\begin{itemize}
\item \textbf{对瓶颈单台设备（$n_r=1$）}：聚合与个体建模完全等价，不引入任何保守性。
\item \textbf{对多台非瓶颈设备（$n_r>1$）}：聚合不允许同类设备”分兵”同时服务不同车间的不同工序（即禁止一台去 A 车间、另一台去 B 车间并行作业）。在本问题结构下，非瓶颈设备的作业时长远小于瓶颈设备，其分兵并行所能节省的时间被瓶颈设备的持续占用所掩盖，故聚合带来的保守性极小。\textbf{5.5 节}的精细对比验证表明，该保守性不超过 $0.16\%$。
\item \textbf{对转运的影响}：聚合超级机器的跨车间转运由各物理设备并行移动完成，日历时间仅增加一次 $\tau_{w,w'}$（详见假设 (7)），这与逐台独立移动、速度相同的前提相容。
\end{itemize}
该聚合使决策变量由逐台设备的个体路由（$\sum_r n_r$ 条回路）压缩为逐类型的编组路由（$|\mathcal{R}|=5$ 条回路），大幅降低了模型规模。
\item 转运按单台物理设备独立计取：题目假定”同一台设备”跨车间转移须计入运输时间，$\tau_{w,w'}=\lceil d_{w,w'}/2\rceil$（s），与同类其他设备是否同时移动无关；同一台设备在同一车间不同工序间的转移时间忽略不计。
\item 在超级机器编组作业口径下，同类 $n_r$ 台设备在同一工序上并行作业、在同一时刻切换至下一作业车间时，各台物理设备并行转场，日历时间仅增加一次 $\tau_{w,w'}$，而非按台数累加；该处理与单台独立移动、速度相同相容，亦不同于将多类设备绑定的整车运输。
...
若对每一台物理设备单独建立路由，决策规模将急剧膨胀，且与瓶颈结构（单台抛光机、单台传感机）不相称。由假设 (3) 与假设 (5)，将类型 $r$ 的 $n_r$ 台设备聚合为容量 $n_r$ 的超级机器，在工序 $o$ 上占用 $p_{o,r}$。

\textbf{转运口径}：按假设 (6)--(7)，转运由单台物理设备独立完成。在编组作业方案中，当类型 $r$ 由车间 $w$ 的工序 $o$ 切换至车间 $w'$ 的工序 $o'$ 时，各物理设备并行转场，日历约束为
...
\item \textbf{跨车间转运计入}：当 $w(o)\neq w(o')$ 时，$\tau_{w(o),w(o')}>0$ 强制预留转运时间；当 $w(o)=w(o')$ 时，$\tau=0$，体现假设 (6)”同车间不同工序间转移时间忽略”。
\end{itemize}
...
本文采用两层混合策略：GA $\rightarrow$ SA $\rightarrow$ \texttt{validate} 得可行上界 $C_{\max}^{\mathrm{ub}}$ 与 $\{s_o\}$；经校验后以 \texttt{AddHint} 注入 $S_i$、$E_i$、$C_{\max}$ 及路由弧，引导 CP-SAT\cite{sun2024} 加速零间隙证明。元启发式不是最终答案来源；Hint 不将 $C_{\max}\le C_{\max}^{\mathrm{ub}}$ 设为硬约束。
...
\begin{figure}[H]
\centering
\includegraphics[width=0.92\textwidth]{figs/pox.png}
\caption{POX 交叉操作示意图（示例：5 个车间、染色体长度 8，$\mathcal{J}=\{\mathrm{A},\mathrm{C}\}$。步骤 1 将父代 $P_1$ 中属于 $\mathcal{J}$ 的基因原位复制到子代；步骤 2 将父代 $P_2$ 中不属于 $\mathcal{J}$ 的基因按原有顺序依次填入子代空位，得到完整子代染色体 $C$）}\label{fig:pox}
\end{figure}
...
\item \textbf{加速效果}。在问题二至四上，热启动（注入 Hint）相比冷启动（不注入 Hint）的 CP-SAT 求解耗时加速比约 $1.4\times$--$1.8\times$（见表~\ref{tab:coldhot}）。对于绝对求解时间较长（$>10\,\mathrm{s}$）的中大规模调度实例，Hint 降低搜索盲目性的收益尤为显著。本文所涉问题因模型规模紧凑（$|\mathcal{O}|=34$），冷启动已可在亚秒级收敛到最优解，故加速比的绝对值有限；但 Hint 机制本身在大规模变体或更严苛时限下是不可或缺的求解效率保障。
\end{enumerate}
...
\]
该方案使高速抛光机与自动传感多功能机均增至 $5$ 台，瓶颈载荷下界降至 $27000\,\mathrm{s}$，求解所得 $32608\,\mathrm{s}$ 与之比为 $1.207$。相较问题三的 $73575\,\mathrm{s}$，总工期缩短约 $55.7\%$。购置组合对总工期的影响见图~\ref{fig:buy}，可见抛光与传感能力需\emph{均衡}扩充方能最大化收益（如 $z_{\text{抛}}=4,z_{\text{传}}=2$ 费用 $460000$ 元仅得 $36255\,\mathrm{s}$）。

\medskip
\noindent\textbf{增购设备在班组间的分配说明}。本文对设备采用资源池化抽象（见 5.5 节模型假设 (3)(6)）：同类设备无论原始归属哪个班组，均合并为统一资源池参与调度。在此框架下，模型的唯一输入参数为该类设备的\textbf{总台数} $n_r$，调度结果 $C_{\max}$ 仅取决于总台数，与设备具体部署在哪个班组无关。换言之，只要增购后高速抛光机总台数达到 $5$、自动传感多功能机总台数达到 $5$，无论 $3$ 台增购设备分配为 $(2,1)$、$(1,2)$ 或 $(3,0)$（括号内为班组 1、班组 2 各自获得的增购台数），传入求解器的 \texttt{counts} 参数完全一致，所得 $C_{\max}^{(4)}=32608\,\mathrm{s}$ 不变。表~\ref{tab:q5} 中填写的分配方案（班组 1 各购 2 台、班组 2 各购 1 台）仅为一种可行的拆分方式，非由优化确定，亦不影响最优性与调度明细。购置方案见表~\ref{tab:q5}，调度甘特图见图~\ref{fig:gq4}，详细调度见表~\ref{tab:q4}。
...
问题二至四均采用两层混合求解框架：GA/SA 生成可行上界，CP-SAT 以 Hint 热启动完成零间隙证明。本节从参数调优、冷/热启动对比及最终求解结果三个层面呈现混合求解的性能。

\subsection{元启发式参数调优}

为确定 GA 与 SA 的最优参数组合，在问题二上对 GA 与 SA 的关键参数进行网格搜索。GA 搜索参数为种群规模 $N_{\mathrm{pop}}\in\{80,150,200\}$、迭代代数 $G\in\{400,800\}$、变异概率 $p_m\in\{0.15,0.2\}$；SA 搜索参数为初温 $T_0\in\{3000,4000\}$、冷却系数 $\alpha\in\{0.95,0.97\}$、每温迭代次数 $N_{\mathrm{iter}}\in\{300,400\}$。每组参数组合下，依次运行 GA $\rightarrow$ SA 级联生成上界，再分别执行 CP-SAT 冷启动与热启动求解。以元启发式上界与已知最优值 $144880\,\mathrm{s}$ 的间隙为首要评价指标、总求解耗时为次要指标进行排序，节选结果见表~\ref{tab:tune}。
...
\midrule
150/400/0.2 & 4000/0.97/300 & 145801 & 921 & 7.587 & 0.219 & 0.146 & 1.495 \\
\textbf{150/800/0.2} & \textbf{4000/0.97/400} & \textbf{144880} & \textbf{0} & \textbf{12.895} & \textbf{0.254} & \textbf{0.133} & \textbf{1.906} \\
200/800/0.15 & 4000/0.97/400 & 144880 & 0 & 13.740 & 0.229 & 0.129 & 1.767 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{推荐参数}：$N_{\mathrm{pop}}=150$，$G=800$，$p_m=0.2$，$T_0=4000$，$\alpha=0.97$，$N_{\mathrm{iter}}=400$。该组合在 GA+SA 耗时 $12.895\,\mathrm{s}$ 内稳定生成零间隙上界，热启动 CP-SAT 仅需 $0.133\,\mathrm{s}$ 即完成最优性证明。增大种群与代数至 $200/800$（搭配 $p_m=0.15$）虽可进一步缩短热启动耗时至 $0.129\,\mathrm{s}$，但元启发式耗时增至 $13.740\,\mathrm{s}$，总耗时反超推荐参数，故不采纳。

\subsection{冷启动与热启动对比}

为量化 Hint 热启动对 CP-SAT 求解的加速效果，对问题二至四分别执行冷启动（不注入 Hint）与热启动（注入 GA/SA 生成的开工时刻、完工时刻与路由弧 Hint），记录各自求解耗时与所得最优值，结果见表~\ref{tab:coldhot}。

\begin{table}[H]
\centering
\caption{冷启动与热启动 CP-SAT 求解耗时对比}\label{tab:coldhot}
\begin{tabular}{c c c c c c}
\toprule
问题 & 冷启动求解耗时(s) & 热启动求解耗时(s) & 加速比 & CP-SAT 最优值(s) & 最优值一致 \\
\midrule
二 & 0.227 & 0.164 & 1.386 & 144880 & 是 \\
三 & 0.215 & 0.144 & 1.494 & 73575 & 是 \\
四 & 0.230 & 0.130 & 1.774 & 32608 & 是 \\
\bottomrule
\end{tabular}
\end{table}

由表~\ref{tab:coldhot} 可见：三个问题的冷/热启动所得最优值完全一致，验证了 Hint 仅引导搜索方向、不裁剪可行解空间的性质；热启动求解耗时均短于冷启动，加速比为 $1.4\times$--$1.8\times$。

\medskip
\noindent\textbf{注：}表~\ref{tab:coldhot} 中问题二的冷/热耗时（$0.227\,\mathrm{s}$ / $0.164\,\mathrm{s}$）与表~\ref{tab:tune} 推荐参数行的对应值（$0.254\,\mathrm{s}$ / $0.133\,\mathrm{s}$）存在差异，原因在于两表的数据来源于独立实验批次：表~\ref{tab:tune} 系 96 组网格搜索中单次运行的计时，表~\ref{tab:coldhot} 系 \texttt{cold\_hot\_table()} 重新执行 \texttt{solve\_hybrid} 所得的计时。两次运行的元启发式随机种子、Hint 编码以及 CP-SAT 求解器内部状态均不相同，导致求解耗时在 $10^{-2}$ 秒量级产生随机波动。该波动不影响最优值（均收敛至 $144880\,\mathrm{s}$ 且零间隙），属亚秒级 CP-SAT 的正常行为。

由于本文所涉问题规模紧凑（$|\mathcal{O}|=34$），CP-SAT 冷启动已可在亚秒级收敛，故加速比的绝对值有限。但对于更大规模或更严苛时限下的调度实例，Hint 热启动降低搜索盲目性的收益将更为显著。

\subsection{混合求解最终结果}

\textbf{【编程求解】}\,冷/热启动对比与混合求解最终对照结果汇总于表~\ref{tab:cv}（由 \texttt{cross\_validate.py} 与 \texttt{validate.py} 输出）。

\begin{table}[H]
\centering
\caption{混合求解最终结果对照}\label{tab:cv}
\begin{tabular}{c c c c c c}
\toprule
问题 & 元启发式上界(s) & CP-SAT 精确值(s) & 上界间隙(s) & 可行性校验 & 冷/热加速比 \\
\midrule
二 & $144880$ & $144880$ & $0$ & 通过 & $1.386$ \\
三 & $73575$ & $73575$ & $0$ & 通过 & $1.494$ \\
四 & $32608$ & $32608$ & $0$ & 通过 & $1.774$ \\
\bottomrule
\end{tabular}
\end{table}

GA 与 SA 具有随机性：固定推荐参数下，部分随机种子仍可能收敛至次优值（如个别种子给出 $145801\,\mathrm{s}$ 的上界，间隙 $921\,\mathrm{s}$），但多次运行可稳定复现精确最优值。本文以 CP-SAT 零间隙证明为最优性的最终依据——元启发式层提供上界与搜索加速，CP-SAT 承担精确证明职责。所有调度结果均经独立可行性校验器 \texttt{validate.py} 逐项核验（先后约束、设备不重叠、转运充足、双设备完工），确保无"表观最短但不可行"的解。各问设备利用率热力图见图~\ref{fig:util}。
...
聚合抽象将同类多台设备约束为成组调度，相对允许个体设备在多道并行工序间自由细分的更精细模型理论上偏保守。然而，在本问题的瓶颈主导结构下，总完工时间由高速抛光机与自动传感多功能机两类瓶颈资源决定（其台数分别为 1 或 2 台，聚合与个体建模等价），输送臂、清洗机、灌装机等非瓶颈资源即便改为逐台独立路由，仅能减少其自身空闲时间，无法使瓶颈设备更快完成作业，故聚合假设对 $C_{\max}$ 不构成实质性影响。在瓶颈设备数量较多、非瓶颈资源亦可能成为次瓶颈的更大规模场景中，可建立以累积资源刻画容量、以个体机器刻画路由的精细模型，以释放聚合假设可能遮蔽的并行收益。此外，本文假设设备无故障、转运速度恒定，未计入随机扰动；后续可在确定性最优解的基础上引入随机作业时间或转运时间，发展具有鲁棒性的调度策略。

\bigskip
\noindent\textbf{关于最优调度方案非唯一性的说明}。需要特别指出的是，CP-SAT 返回 \texttt{OPTIMAL} 状态且目标间隙为零，证明了本文给出的各问 makespan 值（$37280\,\mathrm{s}$、$144880\,\mathrm{s}$、$73575\,\mathrm{s}$、$32608\,\mathrm{s}$）均为\textbf{唯一的最优值}——不存在任何可行调度能使总完工时间严格低于这些数值。然而，达到该最优值的\textbf{具体调度方案（甘特图中各工序的起止时刻与设备分配）并不唯一}，其非唯一性来源于三个层面：

\begin{enumerate}[leftmargin=2.2em, label=(\arabic*)]
\item \textbf{非瓶颈设备的空闲窗口滑动}。当某工序由两类设备协同完成且其中一类的作业时间显著短于另一类时（如 Q1 的 A2 工序，抛光机需 $18000\,\mathrm{s}$ 而清洗机仅需 $1440\,\mathrm{s}$），短时设备在其分内作业完成后存在长达 $16560\,\mathrm{s}$ 的空闲窗口。该作业区间可在窗口内任意平移而不改变工序完工时刻，从而不影响 makespan。

\item \textbf{瓶颈设备间局部路由重排}。瓶颈设备（如高速抛光机）需依次访问多个车间的同类工序。当相邻两段作业之间的转运间隙大于零时，若存在两道不共享任何协同设备的工序，交换其访问顺序而总转运时间不变，则产生另一个等价最优路由。此类交换不影响任何工序链的关键路径。

\item \textbf{CP-SAT 求解器的返回策略}。CP-SAT 在证明最优性后即终止搜索，返回第一个被确认为可行的最优解，而非枚举全部最优解。在热启动模式下，\texttt{AddHint} 会将搜索引导至 GA/SA 找到的特定解附近，CP-SAT 优先验证该解的可行性，因此甘特图展示的调度方案实际上反映了元启发式层的搜索偏好。
\end{enumerate}

\noindent 综上所述，本文所列的甘特图与调度明细表展示的是\textbf{达到最优 makespan 的其中一种可行调度方案}，而非声称其为唯一的调度方式。在工程实践中，若存在多种等价最优方案，可进一步以设备总转运距离、总空闲率等次要指标进行方案优选。
...
\bibitem{shi2026}
时晓钊, 卢一玮, 黄觉安, 韩达稳, 刘志亮.
新生产模式下机加车间调度问题的建模求解算法研究[J].
建模与仿真, 2026, 15(4): 45--54.
DOI: 10.12677/mos.2026.154052.

\bibitem{sun2024}
孙淑军, 李德明, 王长通, 尹世昌, 朱雪莲, 赵文耿.
CP-SAT模型驱动的研发台架任务调度优化策略[J].
创新技术应用, 2024(6): 123--128.
...
本项目所有支撑材料文件整理如表~\ref{tab:filelist} 所示。全部程序基于 Python 3.11 编写，依赖 OR-Tools 9.x（CP-SAT 求解器）、NumPy、Pandas、Matplotlib 等开源库。

\begin{table}[H]
\centering
\caption{支撑材料文件列表}\label{tab:filelist}
\small
\begin{tabular}{p{0.8cm} p{4.5cm} p{2.5cm} p{6.2cm}}
\toprule
\textbf{序号} & \textbf{文件名} & \textbf{类型} & \textbf{功能说明} \\
\midrule
1 & \texttt{B-附件.xlsx} & 数据文件 & 赛题原始数据（工序流程、班组配置、车间距离） \\
2 & \texttt{solver.py} & Python 程序 & CP-SAT 核心求解器，含数据定义与统一求解接口 \\
3 & \texttt{heuristic.py} & Python 程序 & 元启发式层：GA、SA 算法实现与路由弧提取 \\
4 & \texttt{hybrid\_solve.py} & Python 程序 & 两层混合求解框架入口；Q4 购置组合枚举 \\
5 & \texttt{tune\_params.py} & Python 程序 & GA/SA 超参数网格搜索与冷/热启动对比 \\
6 & \texttt{validate.py} & Python 程序 & 独立可行性校验器（先后、不重叠、转运、协同完工） \\
7 & \texttt{extract.py} & Python 程序 & 从最优解提取逐设备调度明细（生成表 1--表 4） \\
8 & \texttt{ga\_q2.py} & Python 程序 & 遗传算法独立调用脚本（问题二） \\
9 & \texttt{sa\_q2.py} & Python 程序 & 模拟退火独立调用脚本（问题二） \\
10 & \texttt{cross\_validate.py} & Python 程序 & 混合求解结果对照与一致性验证 \\
11 & \texttt{make\_figs.py} & Python 程序 & 甘特图与热力图生成程序 \\
12 & \texttt{tune\_results.json} & 结果文件 & 超参数网格搜索结果原始数据 \\
13 & \texttt{cold\_hot.json} & 结果文件 & 冷/热启动 CP-SAT 耗时对比原始数据 \\
14 & \texttt{figs/gantt\_q2.png} & 图像文件 & 问题二调度甘特图 \\
15 & \texttt{figs/gantt\_q3.png} & 图像文件 & 问题三调度甘特图 \\
16 & \texttt{figs/gantt\_q4.png} & 图像文件 & 问题四调度甘特图 \\
17 & \texttt{figs/load\_heatmap.png} & 图像文件 & 各类设备作业载荷热力图 \\
18 & \texttt{figs/util\_heatmap.png} & 图像文件 & 各问设备利用率热力图 \\
19 & \texttt{figs/buy\_heatmap.png} & 图像文件 & 购置组合对总工期影响热力图 \\
\bottomrule
\end{tabular}
\end{table}

%==================================================================
\section{附录 B\quad 各程序的初值选取与参数设置}

\subsection*{B.1 遗传算法（GA）参数设置}

GA 采用基于工序的排列编码，染色体长度为各车间工序数之和（问题二至四为 34）。各参数经网格搜索确定（见正文表~\ref{tab:tune}），详细设置见表~\ref{tab:ga-params}。

\begin{table}[H]
\centering
\caption{遗传算法参数设置}\label{tab:ga-params}
\begin{tabular}{p{4.5cm} p{3.5cm} p{6.5cm}}
\toprule
\textbf{参数} & \textbf{取值} & \textbf{说明} \\
\midrule
种群规模 $N_{\mathrm{pop}}$ & 150 & 每代保留的染色体数量 \\
最大迭代代数 $G$ & 800 & 终止条件；问题二经实验 800 代收敛稳定 \\
变异概率 $p_m$ & 0.2 & 每个子代染色体执行对换变异的概率 \\
锦标赛规模 $k$ & 3 & 每次从种群随机抽取 3 个个体，择优作为父代 \\
交叉方式 & POX & 优先保留顺序交叉，保证子代多重集约束 \\
精英保留 & 1（最优个体） & 每代最优染色体直接进入下一代 \\
\midrule
\multicolumn{3}{l}{\textbf{初值选取}} \\
\midrule
初始种群 & 随机生成 & 对模板向量 $[0,0,0,1,1,1,1,2,\dots,4,4,4]$ 执行 \texttt{random.shuffle} \\
随机种子 & 0 & 固定种子以确保结果可复现 \\
\bottomrule
\end{tabular}
\end{table}

\subsection*{B.2 模拟退火（SA）参数设置}

SA 以 GA 最优染色体为初始解，通过 Metropolis 准则进行概率性局部搜索。参数设置见表~\ref{tab:sa-params}。

\begin{table}[H]
\centering
\caption{模拟退火参数设置}\label{tab:sa-params}
\begin{tabular}{p{4.5cm} p{3.5cm} p{6.5cm}}
\toprule
\textbf{参数} & \textbf{取值} & \textbf{说明} \\
\midrule
初始温度 $T_0$ & 4000 & 高温保证初期接受劣解能力，充分探索解空间 \\
终止温度 $T_{\mathrm{end}}$ & $10^{-2}$ & 低温阶段趋于贪婪搜索 \\
冷却系数 $\alpha$ & 0.97 & 指数退火 $T\leftarrow\alpha T$；总温度级数约 424 级 \\
每温迭代次数 $N_{\mathrm{iter}}$ & 400 & 每个温度下邻域搜索次数；总尝试约 $1.7\times 10^5$ 次 \\
邻域结构 & Swap / Reinsert & 各以 0.5 概率选取；Swap 交换两个基因位置，Reinsert 取出一个基因后随机插入 \\
\midrule
\multicolumn{3}{l}{\textbf{初值选取}} \\
\midrule
初始解 & GA 最优染色体 & 染色体由 GA 800 代最优个体传入；若 GA 失败则随机生成 \\
随机种子 & 0 & 固定种子以确保结果可复现 \\
\bottomrule
\end{tabular}
\end{table}

\subsection*{B.3 CP-SAT 求解器参数设置}

CP-SAT 作为精确求解层，接收元启发式提供的 Hint 进行热启动。参数设置见表~\ref{tab:cpsat-params}。

\begin{table}[H]
\centering
\caption{CP-SAT 求解器参数设置}\label{tab:cpsat-params}
\begin{tabular}{p{4.5cm} p{4.0cm} p{6.0cm}}
\toprule
\textbf{参数} & \textbf{取值} & \textbf{说明} \\
\midrule
并行搜索线程数 & 8 & \texttt{num\_search\_workers}=8，充分利用多核 CPU \\
求解时限（问题一） & 20 s & 单车间规模极小，秒级可证最优 \\
求解时限（问题二） & 120 s & 冷/热启动均设为 120 s \\
求解时限（问题三） & 180 s & 资源池化后搜索空间增大 \\
求解时限（问题四/单组合） & 40 s & 外层枚举多个购置组合，内层设 40 s 控制总耗时 \\
时间上界 $H$ & $\sum\max(p_{o,r})+50000$ & 约 $2\times 10^5$ s，远大于最优值，确保不截断可行域 \\
大 $M$ 常数 & $=H$ & 取时间上界，确保析取约束在弧未激活时自动松弛 \\
\midrule
\multicolumn{3}{l}{\textbf{Hint 热启动设置}} \\
\midrule
Hint 来源 & GA $\rightarrow$ SA 最优解 & 经 \texttt{validate.py} 可行性校验后注入 \\
注入变量 & $S_i$, $E_i$, $C_{\max}$, $x^{r}_{o,o'}$ & 覆盖全部决策变量 \\
Hint 性质 & 搜索引导 & 不设硬约束，不裁剪可行域；CP-SAT 仍可探索更优解 \\
\bottomrule
\end{tabular}
\end{table}

\subsection*{B.4 各问题的参数差异}

不同问题仅在设备台数配置（\texttt{counts}）与基地集合（\texttt{bases}）上存在差异，GA/SA/CP-SAT 核心参数保持一致。各问题的差异化参数见表~\ref{tab:prob-params}。

\begin{table}[H]
\centering
\caption{各问题差异化参数}\label{tab:prob-params}
\begin{tabular}{c c c c c c c c}
\toprule
\textbf{问题} & \textbf{车间} & \textbf{输送臂} & \textbf{清洗机} & \textbf{灌装机} & \textbf{传感机} & \textbf{抛光机} & \textbf{基地} \\
\midrule
一 & \{A\} & 4 & 5 & 5 & 1 & 1 & $\{\mathrm{G1}\}$ \\
二 & \{A,B,C,D,E\} & 4 & 5 & 5 & 1 & 1 & $\{\mathrm{G1}\}$ \\
三 & \{A,B,C,D,E\} & 8 & 10 & 10 & 2 & 2 & $\{\mathrm{G1},\mathrm{G2}\}$ \\
四 & \{A,B,C,D,E\} & 8 & 10 & 10 & 5 & 5 & $\{\mathrm{G1},\mathrm{G2}\}$ \\
\bottomrule
\end{tabular}
\end{table}

%==================================================================
\section{附录 C\quad 求解程序源代码}\label{app:code}
...
%==================================================================
\section{附录 D\quad AI 工具使用详情}\label{app:ai}

本文在赛题分析、数学建模、程序实现与论文撰写过程中，使用了生成式人工智能工具作为\textbf{辅助手段}。AI 的输出均经队员阅读理解、公式核对、程序运行验证及论文统稿后采用；\textbf{AI 工具未参与本队对最终提交稿的独立学术判断与署名决策}，核心建模思路、关键约束设定、最终数值结果与结论由队员负责。

\subsection*{D.1 所用 AI 工具名称与版本}

本队实际使用的 AI 工具仅为 \textbf{Cursor IDE 内置 AI Agent}，对话模型为 \textbf{ChatGPT 5.4}。未使用 ChatGPT、Claude 等网页端大模型，亦未使用 GitHub Copilot 等其他 AI 编程插件。Cursor 主要用于：检索与整理本地项目文件；辅助撰写与扩写 \LaTeX{} 论文；生成并修改 Python 求解程序框架；协助编写图表生成脚本与结果核对脚本；对论文前后逻辑、表图一致性及“评委可能追问点”进行预审式讨论。

\subsection*{D.2 模型与算法的获取情况与渠道}

本文数学模型与求解框架的获取渠道主要包括以下几方面。\textbf{赛题方面}，工序链、设备配置、转运规则、协同完工与预算约束等，来自赛题附件 \texttt{B-附件.xlsx} 与 \texttt{2026-校赛 B.docx}，由队员阅读题意后抽象为统一的“工序链—资源—路由—转运”模型。\textbf{文献方面}，正文已引用两篇与本文方法直接相关的论文：时晓钊等（2026）\cite{shi2026} 为问题二中\textbf{遗传算法与模拟退火级联}生成可行上界提供应用参考；孙淑军等（2024）\cite{sun2024} 为 \textbf{CP-SAT 建模及 \texttt{AddHint} 热启动}混合求解提供方法参考。队员通过阅读上述文献，结合本题约束确定“元启发式上界 + CP-SAT 零间隙证明”的整体路线；GA/SA 编码方式、路由变量构造、Hint 注入及程序实现，在文献思路基础上由本队独立完成并调试。\textbf{其他参考}包括姜启源等《数学模型（第六版）》\cite{jiang2024} 及 OR-Tools 官方文档，用于整数规划表述与 CP-SAT API 调用。上述模型与算法的来源以赛题资料与参考文献为准，\textbf{并非由 AI 直接给出或替代文献调研获得}。

\subsection*{D.3 主要使用目的与代表性提示词}

\textbf{（1）询问某题可能的建模方法。}例如：“多车间 SDST + 协同完工，适合哪些建模框架？” AI 帮助梳理思路；本队结合题设与文献确定以资源聚合 + CP-SAT 为核心、问题四外层枚举购置的结构。

\textbf{（2）让 AI 解释陌生算法或背景知识。}例如：“\texttt{AddCircuit}、\texttt{AddHint} 在调度模型中如何对应路由与热启动？”用于理解已选方法，具体变量定义与约束表述由队员对照程序重写。

\textbf{（3）让 AI 辅助推导公式、检查推导过程。}例如：问题一 $C_{\max}$ 解析式、问题四载荷下界与购置降维是否成立。数值均经 Python 重算并以求解器结果为准。

\textbf{（4）让 AI 比较不同方案的优缺点。}例如：聚合模型与个体机器模型、冷/热启动 CP-SAT 的利弊。供队员决策参考，是否写入正文由人工审定。

\textbf{（5）生成代码框架、定位代码报错。}例如：\texttt{solver.py}、\texttt{heuristic.py}、\texttt{hybrid\_solve.py}、\texttt{validate.py}、\texttt{make\_figs.py} 等模块搭建及报错排查。全部程序经本地运行与对照脚本验证。

\textbf{（6）检查前后逻辑是否矛盾。}例如：假设与池化表述、摘要与附录耗时口径、购置表班组分配与模型是否一致、甘特图与附录表是否对应。

\textbf{（7）扮演评委，专门寻找漏洞和追问。}例如：聚合是否过简、混合求解依据是否充分、表图与复现关系如何表述。用于预演答辩，口径由队员确认。

\subsection*{D.4 采纳情况与人工修改说明}

可采纳算法与软件背景说明、代码结构建议、\LaTeX{} 段落与表格框架、逻辑矛盾检查清单；必须人工完成或核实的内容包括：题设数据录入、模型假设与符号定稿、CP-SAT 零间隙最优性、独立可行性校验、附录调度明细与图表数值、参考文献与全文统稿。混合求解流程与文献方向一致，但 Hint 编码、GA/SA 参数等经多轮实验确定；未经程序验证的耗时、加速比或缺乏推导来源的比例性结论，不作为正文依据。

\subsection*{D.5 声明}

除 Cursor（ChatGPT 5.4）外，本队未使用其他 AI 工具。AI 生成内容不构成独立作者贡献，\textbf{本队队员对提交材料的完整性、正确性与学术规范性承担全部责任}。
